    ...
    async def refresh_mrkt_floors_bulk(self) -> dict[str, int]:
        ...
        try:
            ...
            for c in collections:
                ...
                # Always store by API name (slug)
                if name:
                    floors[name] = floor

                is_numeric_id = bool(name) and name.isdigit()
                if not is_numeric_id:
                    ...
                    # For numeric IDs, only store by title if this is the mapping we use
                    # (prevents rare editions from overwriting standard edition floors)
                    if title and self._title_to_api_name.get(title) == name:
                        floors[title] = floor
